...
string xmlPath = "D:\\openCV\\openCV\\sources\\data\\haarcascades_cuda\\haarcascade_frontalface_default.xml"; 
bool isOpen = true;
...
cv::Mat ProcessImage(cv::Mat frame)
{
    ...
    setting.append("D:\\Git\\cmproject\\QtOpenCv\\QtOpenCv\\sf6.0_windows\\models\\face_detector.csta");
    ...
    for (int i = 0; i < faces.size; i++)
    {
        ...
        rectangle(frame, cv::Rect(face.x, face.y, face.width, face.height), cv::Scalar(0, 0, 255), 2, 8, 0);
    }
...
void TestSeatImage(Ui::QtWidgetsApplication1Class ui)
{
    ...
    QImage Qtemp;
    Mat srcImg, temp;
    srcImg = imread("D:\\Git\\cmproject\\QtOpenCv\\QtOpenCv\\cvtest\\111.jpg");
    if (srcImg.empty())
    {
